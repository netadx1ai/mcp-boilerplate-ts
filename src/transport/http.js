...
export class HttpTransport {
    ...
    onerror;
    onmessage;
    constructor(config = {}, tools = new Map()) {
        this._config = { ...DEFAULT_HTTP_CONFIG, ...config };
        this._logger = createDefaultLogger({ level: 'info', format: 'pretty', output: 'console' }, 'http-transport');
        this._app = express();
        this._tools = tools;
     ...
     */
    _createRequestContext(req, res, next) {
    ...
    _authMiddleware(req, res, next) {
        ...
        const authHeader = req.headers[auth.headerName?.toLowerCase() || 'x-api-key'];
        if (!authHeader) {
            res.status(401).json({
                error: 'Unauthorized',
        ...
        try {
            switch (auth.type) {
                case 'apikey':
                    if (!auth.apiKeys?.includes(authHeader)) {
                ...
                case 'jwt':
                    ...
                    const decoded = jwt.verify(authHeader.replace('Bearer ', ''), auth.jwtSecret);
                    req.user = decoded;
                    break;
                case 'bearer':
                case 'basic':
 ...
 */
export function createHttpTransport(config = {}, tools = new Map()) {
...
export class HttpTransportFactory {
    ...
    static createWithAuth(config = {}, authConfig) {
