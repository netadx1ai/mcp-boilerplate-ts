...
import { z } from 'zod';
import { ServerConfigError, DEFAULT_PORTS, DEFAULT_TIMEOUTS, DEFAULT_LIMITS, ENV_PREFIX, } from '../types/index.js';
 ...
 */
export function createDefaultConfig(serverName) {
 ...
 */
function createDefaultLoggingConfig(environment) {
 ...
 */
function createDefaultSecurityConfig(environment) {
    const isProduction = environment === 'production';
    return {
 ...
 */
function createDefaultPerformanceConfig(environment) {
    const isProduction = environment === 'production';
    return {
 ...
 */
function getDefaultPort(serverName) {
    if (!serverName)
        return 8000;
 ...
 */
export function loadConfigFromEnv(serverName) {
    const config = {};
    // Basic configuration
 ...
 */
export async function loadCompleteConfig(serverName, configFile) {
    try {
        // Start with defaults
 ...
 */
export function getConfigValue(key, defaultValue, envKey) {
    ...
    if (typeof defaultValue === 'boolean') {
        return getEnvBoolean(envKey || key, defaultValue);
    }
    return envValue;
}
// =============================================================================
...
export function validateServerName(name) {
    ...
    if (!/^[a-zA-Z0-9\-_]+$/.test(name)) {
        throw new ServerConfigError('Server name can only contain letters, numbers, hyphens, and underscores');
    }
}
/**
...
export function validateVersion(version) {
    if (!/^\d+\.\d+\.\d+$/.test(version)) {
        throw new ServerConfigError(`Invalid version ${version}: must follow semver format (e.g., 1.0.0)`);
    }
}
// =============================================================================
 ...
 */
export function createDevConfig(serverName) {
 ...
 */
export function createProdConfig(serverName) {
    const base = createDefaultConfig(serverName);
    return {
        ...base,
 ...
 */
export function createTestConfig(serverName) {
    const base = createDefaultConfig(serverName);
    return {
        ...base,
        environment: 'test',
 ...
 * @param pretty - Whether to format JSON prettily
 */
export async function exportConfigToFile(config, filePath, pretty = true) {
    try {
        const fs = await import('fs/promises');
 ...
 * @returns Validated configuration
 */
export async function quickConfig(serverName, options = {}) {
