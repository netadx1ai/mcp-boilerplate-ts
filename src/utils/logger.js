 ...
 */
const LOG_FORMATS = {
    development: format.combine(format.timestamp({ format: 'YYYY-MM-DD HH:mm:ss' }), format.errors({ stack: true }), format.colorize(), format.printf(({ timestamp, level, message, ...meta }) => {
 ...
 */
export function createDefaultLogger(config, serverName) {
    ...
    const logFormat = config.format === 'json'
        ...
        : config.format === 'pretty'
            ...
            : LOG_FORMATS[environment] || LOG_FORMATS.development;
    // Console transport
    ...
    const logger = createLogger({
        levels: LOG_LEVELS,
        level: config.level,
        format: format.combine(format.timestamp(), format.errors({ stack: true }), serverName ? format.label({ label: serverName }) : format.label({ label: 'mcp-server' }), format.metadata({ fillExcept: ['message', 'level', 'timestamp', 'label'] })),
 ...
 */
export function createRequestLogger(logger, requestId, additionalContext = {}) {
 ...
 */
export function createToolLogger(logger, toolName, requestId) {
 ...
 */
export function logTimingDecorator(logger, operation) {
 ...
 */
export function logError(logger, error, context = {}, operation) {
    const errorMessage = error instanceof Error ? error.message : String(error);
 ...
 */
export function logAndThrow(logger, error, context = {}, operation) {
 ...
 */
export function logMetric(logger, metric, value, unit, tags = {}) {
 ...
 * @param logger - Logger instance
 */
export function logSystemMetrics(logger) {
 ...
 * @param context - Additional context
 */
export function logSecurityEvent(logger, event, result, context = {}) {
 ...
 * @param context - Additional context
 */
export function logAuditEvent(logger, action, resource, user, result = 'success', context = {}) {
 ...
 */
export function createAppLogger(appName, level = 'info') {
    const environment = process.env.NODE_ENV || 'development';
    return createLogger({
        level,
        format: LOG_FORMATS[environment] || LOG_FORMATS.development,
        defaultMeta: {
            service: appName,
            environment,
            pid: process.pid,
 ...
 */
export function createSecurityLogger(serviceName) {
    return createLogger({
        level: 'info',
 ...
 */
export function createCorrelationContext(traceId, spanId, parentSpanId) {
    return {
        traceId,
        spanId,
 ...
 */
export function logFunctionEntry(logger, functionName, params = {}) {
 ...
 */
export function logFunctionExit(logger, functionName, result, duration) {
    logger.debug(`← ${functionName}`, {
        function: functionName,
 ...
 */
export function createLogAggregator(logger) {
...
export function createDebugLogger(component) {
    const logger = createLogger({
        level: 'debug',
        format: format.combine(format.timestamp({ format: 'HH:mm:ss.SSS' }), format.colorize(), format.printf(({ timestamp, level, message, ...meta }) => {
 ...
 */
export function createTraceLogger(component) {
 ...
 */
export function logHealthCheck(logger, checkName, status, duration, details) {
...
export function createMockLogger() {
    ...
    mockLogger.log = function (level, message, meta) {
        logs.push({ level: level.level || level, message, meta });
        return originalLog.call(mockLogger, level, message);
    };
    // Add utility methods
