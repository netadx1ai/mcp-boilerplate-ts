...
import { ServerConfigError, ToolExecutionError, ValidationError, DEFAULT_TIMEOUTS, isMcpTool, isToolResult, } from '../types/index.js';
...
export class BaseMcpServer extends EventEmitter {
    ...
    constructor(config) {
        ...
        try {
            ...
            throw new ServerConfigError(`Invalid server configuration: ${error instanceof Error ? error.message : String(error)}`);
        }
    ...
    async stop() {
        ...
        try {
            ...
            await Promise.race([
                this._server.close(),
                new Promise((_, reject) => setTimeout(() => reject(new Error('Shutdown timeout')), DEFAULT_TIMEOUTS.SERVER_SHUTDOWN)),
            ]);
        ...
        this._server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            try {
                ...
                const result = await Promise.race([
                    tool.execute(args ?? {}),
                    new Promise((_, reject) => setTimeout(() => reject(new Error('Tool execution timeout')), DEFAULT_TIMEOUTS.TOOL_EXECUTION)),
                ]);
     ...
     */
    async _createHealthCheck(name, checkFn) {
        const startTime = Date.now();
        try {
 ...
 * @returns Configured MCP server
 */
export async function createBasicServer(name, tools = []) {
